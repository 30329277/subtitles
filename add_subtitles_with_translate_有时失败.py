...
# 不需要配置 FFmpeg 路径，因为已经在系统环境变量中配置了
...
def transcribe_and_translate(video_path, output_path):
    ...
    try:
        with open(subtitles_path, "w", encoding="utf-8") as f:
            for i, (start, end, text) in enumerate(translated_texts, start=1):
                f.write(f"{i}\n")
...
for file_name in os.listdir(input_folder):
    if file_name.endswith((".mp4", ".mkv", ".avi")):
        ...
        output_path = os.path.join(output_folder, os.path.splitext(file_name)[0] + "_zh.mp4")
        try:
